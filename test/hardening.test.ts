...
describe("withLock", () => {
	...
	it("is re-entrant for the same path within one process", () => {
		const dir = tmpDir("ic-lock3-");
		const lockPath = join(dir, "ledger.lock");
		const order: string[] = [];
		withLock(lockPath, () => {
			order.push("outer");
...
describe("p() path resolution", () => {
	it("keeps absolute paths absolute (does not nest under cwd)", () => {
		const abs = join(tmpdir(), "ic-abs-marker");
		expect(p(abs)).toBe(abs);
		expect(p(abs).startsWith(process.cwd())).toBe(false);
	});

	it("resolves relative paths against cwd", () => {
		expect(p("foo", "bar.yml")).toBe(join(process.cwd(), "foo", "bar.yml"));
	});
});
