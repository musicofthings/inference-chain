...
describe("evolveLedger — interaction transitions", () => {
	...
	it("rejected belief moves to rejected_hypotheses (once, not duplicated)", () => {
		const l0 = baseLedger();
		const u = update({
			rejected: [{ belief: "cache works", reason: "stale reads" }],
		});
		const { updatedLedger: l1 } = evolveLedger(
			l0,
			{ kind: "interaction", value: u },
			false,
		);
		const { updatedLedger: l2 } = evolveLedger(
			l1,
			{ kind: "interaction", value: u },
			false,
		);
	...
	it("confirmed belief creates an active hypothesis with supporting evidence", () => {
		...
		expect(updatedLedger.active_hypotheses[0].supporting_evidence).toContain(
			"unit pass",
		);
	});
	...
	it("confirmed twice promotes to stable_learnings", () => {
		...
		const { updatedLedger: l1 } = evolveLedger(
			baseLedger(),
			{ kind: "interaction", value: u1 },
			false,
		);
		const { updatedLedger: l2 } = evolveLedger(
			l1,
			{ kind: "interaction", value: u2 },
			false,
		);
		expect(l2.stable_learnings).toContain("X works");
		expect(
			l2.active_hypotheses.find((h) => h.hypothesis === "X works"),
		).toBeUndefined();
	});
	...
	it("confirming an already-promoted belief is a no-op (no duplicate in active+stable)", () => {
		...
		const u3 = update({
			id: "u3",
			confirmed: [{ belief: "X works", evidence: "C" }],
		});
		let l = baseLedger();
		l = evolveLedger(
			l,
		...
		l = evolveLedger(
			l,
			{ kind: "interaction", value: u3 },
			false,
		).updatedLedger;
		expect(l.stable_learnings).toContain("X works");
		expect(
			l.active_hypotheses.find((h) => h.hypothesis === "X works"),
		).toBeUndefined();
	});
	...
	it("superseded old belief becomes rejected; new belief becomes active", () => {
		...
		expect(
			updatedLedger.rejected_hypotheses.map((r) => r.hypothesis),
		).toContain("use REST");
		expect(updatedLedger.active_hypotheses.map((h) => h.hypothesis)).toContain(
			"use gRPC",
		);
	});
	...
	it("next_action_delta replaces frontier when non-empty; preserved when empty", () => {
		...
		expect(l1.current_frontier.next_best_action).toEqual([
			"write ingest tests",
		]);
		...
		expect(l2.current_frontier.next_best_action).toEqual([
			"write ingest tests",
		]);
	});
});
...
describe("evolveLedger — session transitions", () => {
	...
	it("did_not_work entries become rejected_hypotheses", () => {
		...
		const { updatedLedger } = evolveLedger(
			baseLedger(),
			{ kind: "session", value: b },
			false,
		);
		expect(
			updatedLedger.rejected_hypotheses.map((r) => r.hypothesis),
		).toContain("ad hoc parsing");
	});
	...
	it("issues_identified populate blockers and recurring_failure_patterns", () => {
		...
		expect(updatedLedger.current_frontier.blockers).toContain(
			"flaky test in CI",
		);
		expect(
			updatedLedger.recurring_failure_patterns.map((p) => p.pattern),
		).toContain("flaky test in CI");
	});
	...
	it("fixes_attempted populate stable_decisions", () => {
		...
		expect(updatedLedger.stable_decisions.map((d) => d.decision)).toContain(
...
describe("evolveLedger — MemoryEvolutionRecord output", () => {
	...
	it("frontier_update records previous and new next_best_action", () => {
		...
		expect(evolutionRecord.frontier_update.previous_next_action).toEqual([
			"setup",
		]);
		expect(evolutionRecord.frontier_update.new_next_action).toEqual([
			"ship v0.2",
		]);
	});
});
...
describe("evolveLedger — blocker resolution pruning", () => {
	...
	it("prunes a pre-existing blocker resolved via did_not_work in a session brief", () => {
		...
		const { updatedLedger } = evolveLedger(
			...
			{
				kind: "session",
				value: brief({ did_not_work: ["flaky integration test"] }),
			},
			true,
		);
...
describe("evolveLedger — exclusive belief sections", () => {
	it("reject removes a belief from stable_learnings", () => {
		const l0 = baseLedger();
		l0.stable_learnings = ["cache everything"];
		const { updatedLedger } = evolveLedger(
			l0,
			{
				kind: "interaction",
				value: update({
					rejected: [{ belief: "cache everything", reason: "stale reads" }],
				}),
			},
			false,
		);
		expect(updatedLedger.stable_learnings).not.toContain("cache everything");
		expect(
			updatedLedger.rejected_hypotheses.map((r) => r.hypothesis),
		).toContain("cache everything");
		expect(
			updatedLedger.active_hypotheses.find(
				(h) => h.hypothesis === "cache everything",
			),
		).toBeUndefined();
	});

	it("confirm clears a prior rejection", () => {
		const l0 = baseLedger();
		l0.rejected_hypotheses = [
			{
				hypothesis: "use gRPC",
		...
		const { updatedLedger } = evolveLedger(
			l0,
			{
				kind: "interaction",
				value: update({
					confirmed: [{ belief: "use gRPC", evidence: "latency win" }],
				}),
			},
			false,
		);
		expect(updatedLedger.rejected_hypotheses).toHaveLength(0);
		expect(
			updatedLedger.active_hypotheses.some(
				(h) => h.hypothesis === "use gRPC",
			) || updatedLedger.stable_learnings.includes("use gRPC"),
		).toBe(true);
	});

	it("session did_not_work wins over worked for the same belief", () => {
		const { updatedLedger } = evolveLedger(
			baseLedger(),
			{
				kind: "session",
				value: brief({
					worked: ["ad hoc parsing"],
					did_not_work: ["ad hoc parsing"],
				}),
			},
			false,
		);
		expect(updatedLedger.stable_learnings).not.toContain("ad hoc parsing");
		expect(
			updatedLedger.rejected_hypotheses.map((r) => r.hypothesis),
		).toContain("ad hoc parsing");
	});

	it("promotes on first confirm when stablePromotionThreshold is 1", () => {
		const { updatedLedger, evolutionRecord } = evolveLedger(
			baseLedger(),
			{
				kind: "interaction",
				value: update({
		...
		expect(updatedLedger.stable_learnings).toContain("parser strategy");
		expect(updatedLedger.active_hypotheses).toHaveLength(0);
		expect(evolutionRecord.promoted_to_stable).toHaveLength(1);
	});

	it("anti_repeat_update lists only newly added items", () => {
		const l0 = baseLedger();
		l0.do_not_repeat = ["never mock the DB"];
		const { evolutionRecord, updatedLedger } = evolveLedger(
			l0,
			{
				kind: "interaction",
				value: update({
					do_not_repeat_delta: ["never mock the DB", "skip flaky e2e"],
				}),
			},
			false,
		);
		expect(updatedLedger.do_not_repeat).toEqual([
			"never mock the DB",
			"skip flaky e2e",
		]);
		expect(evolutionRecord.anti_repeat_update).toEqual(["skip flaky e2e"]);
	});
});
