...
const write = (name: string, l: ChainLedger) =>
	writeFileSync(join(dir, name), YAML.stringify(l));
...
describe("mergeTeamLedgersFromDir", () => {
	it("merges dev_*.yml ledgers, derives authors, and surfaces conflicts", () => {
		write(
			...
			ledger({
				iteration: 2,
				stable_learnings: ["Shard GLnexus by chromosome"],
			}),
		);
		write(
			"dev_alex.yml",
			ledger({
				rejected_hypotheses: [
					{
						hypothesis: "shard glnexus by chromosome",
						reason_rejected: "tie",
	...
	it("ignores non-dev yml files", () => {
		...
		write(
			"team-ledger.yml",
			ledger({ stable_learnings: ["SHOULD_BE_IGNORED"] }),
		);
	...
	it("reports an actionable error for a malformed dev ledger", () => {
		writeFileSync(
			join(dir, "dev_broken.yml"),
			"project_id: x\niteration: not-a-number\n",
		);
		expect(() => mergeTeamLedgersFromDir(dir)).toThrow(
			/Invalid developer ledger dev_broken\.yml: iteration/,
		);
	});

	it("throws when no developer ledgers are present", () => {
		mkdirSync(join(dir, "empty"), { recursive: true });
		expect(() => mergeTeamLedgersFromDir(join(dir, "empty"))).toThrow(
			/No developer ledgers/,
		);
	});
});
