...
import { type TeamInput, mergeTeamLedgers } from "../src/teams/merge.js";
...
describe("mergeTeamLedgers", () => {
		...
		const inputs: TeamInput[] = [
			{
				author: "shibi",
				ledger: ledger({
					stable_learnings: ["Shard GLnexus by chromosome", "Pin Python 3.11"],
				}),
			},
			{
				author: "alex",
				ledger: ledger({
					stable_learnings: ["shard glnexus by chromosome", "Use WGS model"],
				}),
			},
		];
	...
	it("detects an assert/deny conflict and quarantines the belief", () => {
		const inputs: TeamInput[] = [
			{
				author: "shibi",
				ledger: ledger({ stable_learnings: ["DeepVariant WGS beats WES"] }),
			},
			{
				author: "alex",
				ledger: ledger({
					rejected_hypotheses: [
						{
							hypothesis: "deepvariant wgs beats wes",
							reason_rejected: "tie on our data",
							rejected_at_iteration: 2,
						},
					],
				}),
			},
		...
		expect(
			teamLedger.open_questions.some((q) =>
				q.startsWith("Resolve team conflict"),
			),
		).toBe(true);
	});
	...
	it("prunes a team blocker that any developer has rejected", () => {
		const inputs: TeamInput[] = [
			...
			},
			{
				...
				ledger: ledger({
					rejected_hypotheses: [
						{
							hypothesis: "GLnexus OOM at 64GB",
							reason_rejected: "fixed by sharding",
							rejected_at_iteration: 3,
						},
					],
				}),
			},
		...
		expect(teamLedger.current_frontier.blockers).not.toContain(
			"GLnexus OOM at 64GB",
		);
		expect(
			teamLedger.rejected_hypotheses.some(
				(r) => r.hypothesis === "GLnexus OOM at 64GB",
			),
		).toBe(true);
	});
	...
	it("refuses to merge ledgers with different project_id values", () => {
		const inputs: TeamInput[] = [
			{ author: "shibi", ledger: ledger({ project_id: "alpha" }) },
			{ author: "alex", ledger: ledger({ project_id: "beta" }) },
		];
		expect(() => mergeTeamLedgers(inputs)).toThrow(/different project_id/);
	});
});
