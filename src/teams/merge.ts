...
export function mergeTeamLedgers(inputs: TeamInput[]): TeamMergeResult {
	if (inputs.length === 0)
		throw new Error("mergeTeamLedgers requires at least one ledger.");

	// Stable author order for deterministic tie-breaks.
	const devs = [...inputs].sort((a, b) =>
		a.author < b.author ? -1 : a.author > b.author ? 1 : 0,
	);

	const projectIds = [...new Set(devs.map((d) => d.ledger.project_id))];
	if (projectIds.length > 1) {
		throw new Error(
			`Cannot merge ledgers with different project_id values: ${projectIds.join(", ")}.`,
		);
	}
	...
	for (const { author, ledger } of devs) {
		...
		for (const h of ledger.active_hypotheses)
			record(asserts, h.hypothesis, author);
		for (const r of ledger.rejected_hypotheses)
			record(denies, r.hypothesis, author);
	}
	...
	for (const { ledger } of devs) {
		for (const h of ledger.active_hypotheses) {
			...
			if (!cur) {
				...
				cur.supporting_evidence = sortedUnique([
					...cur.supporting_evidence,
					...h.supporting_evidence,
				]);
				cur.contradicting_evidence = sortedUnique([
					...cur.contradicting_evidence,
					...h.contradicting_evidence,
				]);
				if (CONF_RANK[h.confidence] > CONF_RANK[cur.confidence])
					cur.confidence = h.confidence;
				cur.first_seen_at_iteration = Math.min(
					cur.first_seen_at_iteration,
					h.first_seen_at_iteration,
				);
			}
		}
	}
	...
	for (const { ledger } of devs) {
		for (const d of ledger.stable_decisions) {
			...
			if (!cur) {
				...
				cur.first_introduced_at_iteration = Math.min(
					cur.first_introduced_at_iteration,
					d.first_introduced_at_iteration,
				);
				cur.last_confirmed_at_iteration = Math.max(
					cur.last_confirmed_at_iteration,
					d.last_confirmed_at_iteration,
				);
				if (CONF_RANK[d.confidence] > CONF_RANK[cur.confidence])
	...
	const patMap = new Map<
		string,
		ChainLedger["recurring_failure_patterns"][number]
	>();
	for (const { ledger } of devs) {
	...
	const rejectedAll = new Map<
		string,
		ChainLedger["rejected_hypotheses"][number]
	>();
	for (const { ledger } of devs) {
		for (const r of ledger.rejected_hypotheses) {
			...
			if (!cur) rejectedAll.set(k, { ...r });
				...
				cur.rejected_at_iteration = Math.max(
					cur.rejected_at_iteration,
					r.rejected_at_iteration,
				);
		}
	}
	...
	const risks = sortedUnique(
		devs.flatMap((d) => d.ledger.current_frontier.risks),
	);
