...
import { eventCount, openDb } from "./storage/sqlite.js";
...
function initFresh(projectName: string): void {
	initProject({ projectName, force: true, via: "simulate" });
}
...
function diffRejected(
	...
	return after
		.filter((r) => !beforeSet.has(norm(r.hypothesis)))
		.map((r) => r.hypothesis);
}

function color(
	c: "green" | "red" | "yellow" | "cyan" | "dim",
	s: string,
): string {
...
export async function runSimulation(opts: {
	...
	try {
		for (let i = 0; i < files.length; i++) {
			...
			const raw = YAML.parse(readFileSync(fullPath, "utf8")) as {
				kind?: string;
			};
			...
			const step: StepRecord = {
				...
				added_stable: diffStrings(
					before.stable_learnings,
					after.stable_learnings,
				),
				added_rejected: diffRejected(
					before.rejected_hypotheses,
					after.rejected_hypotheses,
				),
				added_do_not_repeat: diffStrings(
					before.do_not_repeat,
					after.do_not_repeat,
				),
		...
		writeFileAtomic(PATHS.resumeLatest(), briefText);
		appendChainEvent(db, {
	...
	const halfwayIteration = Math.floor(
		(finalLedger.iteration + startIteration) / 2,
	);
	...
	const rejectedSet = new Set(
		finalLedger.rejected_hypotheses.map((r) => norm(r.hypothesis)),
	);
	...
	const scoreProgression =
		iterationsAdvanced === 0
			? finalScore - startScore
			: (finalScore - startScore) / iterationsAdvanced;

	const briefSizeKb = Buffer.byteLength(briefText, "utf8") / 1024;

	const positive =
		antiRepeatCoverage >= 0.5 &&
		rejectedPersistence === 0 &&
		scoreProgression > 0;
	const notes: string[] = [];
	...
	if (!verify.ok)
		notes.push(
			`chain verification FAILED with ${verify.errors.length} error(s)`,
		);
...
function printStep(s: StepRecord): void {
	...
	if (s.added_rejected.length)
		console.log(
			`  ${color("red", "rejected+")} ${s.added_rejected.join(" | ")}`,
		);
	if (s.added_do_not_repeat.length)
		console.log(
			`  ${color("yellow", "do-not-repeat+")} ${s.added_do_not_repeat.join(" | ")}`,
		);
	...
	if (
		...
		console.log(
			`  ${color("cyan", "frontier:")} ${s.frontier_after.join(" | ")}`,
		);
	}
}
...
function printReport(r: SimulationReport): void {
	...
	console.log("");
	console.log(
		`anti_repeat_coverage      ${r.metrics.anti_repeat_coverage}  ${gate(r.metrics.anti_repeat_coverage >= 0.5)}`,
	);
	console.log(
		`hypothesis_promotion_rate ${r.metrics.hypothesis_promotion_rate}`,
	);
	console.log(`frontier_convergence      ${r.metrics.frontier_convergence}`);
	console.log(
		`rejected_persistence      ${r.metrics.rejected_persistence}  ${gate(r.metrics.rejected_persistence === 0)}`,
	);
	console.log(
		`score_progression         ${r.metrics.score_progression}  ${gate(r.metrics.score_progression > 0)}`,
	);
