import type { ChainLedger, InteractionUpdate, SessionBrief } from './schemas.js';
...
export function evolveLedger(previous: ChainLedger, source: Source, advance: boolean): ChainLedger {
  ...
  if (source.kind === 'session') {
    ...
    next.current_frontier.next_best_action =
      brief.next_best_action.length > 0
        ...
        : next.current_frontier.next_best_action;
    next.do_not_repeat = Array.from(new Set([...next.do_not_repeat, ...brief.do_not_repeat]));

    // Treat "worked" outcomes as reinforcement-style stable learnings.
    next.stable_learnings = Array.from(
      new Set([...next.stable_learnings, ...brief.worked, ...brief.outcomes_observed]),
    );
