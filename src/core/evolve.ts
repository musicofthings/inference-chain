...
export function evolveLedger(
	...
	// Beliefs occupy at most one exclusive section: active | stable | rejected.
	const dropFromActive = (belief: string) => {
	...
	const tryPromote = (h: ActiveHypothesis, idx: number) => {
		if (h.supporting_evidence.length < stablePromotionThreshold) return;
		promoted.push({
	...
	const applyConfirmed = (belief: string, evidence: string) => {
		...
		if (idx >= 0) {
			...
			// Threshold 1 (or any threshold already met by the first evidence item)
			// must promote immediately — not only on a later re-confirm path.
	...
	if (source.kind === "session") {
		...
		// worked → exclusive stable learnings (durable wins)
		for (const w of brief.worked) assertStable(w);

		// partially_worked → candidate active hypotheses (exclusive)
		...
		// did_not_work → rejected (wins over worked if both list the same belief)
	...
	const evolutionRecord: MemoryEvolutionRecord = {
		...
		source: source.kind === "session" ? "session_brief" : "interaction_update",
		new_information: Array.from(
			new Set(newInformation.map((s) => s.trim())),
		).filter(Boolean),
		...
		anti_repeat_update: Array.from(
			new Set(antiRepeatAdded.map((s) => s.trim())),
		).filter(Boolean),
